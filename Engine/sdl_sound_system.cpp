...
namespace
{
    enum class sound_event_type { play, load, quit };
...
struct sdl_sound_system::impl
{
    ...
    {
        { std::lock_guard<std::mutex> lock(mutex); event_queue.push(std::move(evt)); }
        cv.notify_one();
    }

    void load(sound_id id, const std::string& path)
    {
        enqueue({ sound_event_type::load, id, 1.0f, path });
    }

    void play(sound_id id, float volume)
    {
        if (m_Muted) return;
        enqueue({ sound_event_type::play, id, volume, {} });
    }
    ...
    void process_loop()
    {
        while (true)
        {
            ...
            switch (evt.type)
            {
            case sound_event_type::quit: return;

            case sound_event_type::load:
            {
                if (!mixer) { std::cerr << "mixer not initialized\n"; break; }
                if (audio_clips.contains(evt.id)) break;
...
void sdl_sound_system::play(const sound_id id, const float volume) { _impl->play(id, volume); }
void sdl_sound_system::load(const sound_id id, const std::string& path) { _impl->load(id, path); }
void sdl_sound_system::SetMuted(bool muted) { _impl->m_Muted = muted; }
bool sdl_sound_system::IsMuted() const { return _impl->m_Muted; }