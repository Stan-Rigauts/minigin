...
namespace dae
{
    struct InputManager::Impl
    {
        ...
        std::unordered_map<WORD, std::unique_ptr<Command>> m_ButtonBindings{};
        std::unordered_map<SDL_Scancode, std::unique_ptr<Command>> m_KeyBindings{};
#ifdef __EMSCRIPTEN__
    ...
    bool InputManager::ProcessInput()
    {
        ...
        for (auto& [scancode, command] : m_pImpl->m_KeyBindings)
            if (keyState[scancode] && command) command->Execute();
        ...
        if (m_pImpl->m_SDLController)
        {
            ...
            m_pImpl->m_CurrentState.Gamepad.sThumbLX = (short)SDL_GetGamepadAxis(m_pImpl->m_SDLController, SDL_GAMEPAD_AXIS_LEFTX);
            m_pImpl->m_CurrentState.Gamepad.sThumbLY = (short)SDL_GetGamepadAxis(m_pImpl->m_SDLController, SDL_GAMEPAD_AXIS_LEFTY);
        }
#else
        DWORD result = XInputGetState(0, &m_pImpl->m_CurrentState);
        if (result != ERROR_SUCCESS) m_pImpl->m_CurrentState = {};
#endif

        auto buttonChanges = m_pImpl->m_CurrentState.Gamepad.wButtons ^ m_pImpl->m_PreviousState.Gamepad.wButtons;
        m_pImpl->m_ButtonsPressedThisFrame = buttonChanges & m_pImpl->m_CurrentState.Gamepad.wButtons;
        m_pImpl->m_ButtonsReleasedThisFrame = buttonChanges & (~m_pImpl->m_CurrentState.Gamepad.wButtons);

        for (auto& [button, command] : m_pImpl->m_ButtonBindings)
            if (IsPressed(static_cast<ControllerButton>(button)) && command) command->Execute();
        return true;
    }
    ...
    {
        return (m_pImpl->m_ButtonsPressedThisFrame & static_cast<unsigned short>(button)) != 0;
    }

    bool InputManager::IsUpThisFrame(ControllerButton button) const
    {
        return (m_pImpl->m_ButtonsReleasedThisFrame & static_cast<unsigned short>(button)) != 0;
    }

    bool InputManager::IsPressed(ControllerButton button) const
    {
        return (m_pImpl->m_CurrentState.Gamepad.wButtons & static_cast<unsigned short>(button)) != 0;
    }

    void InputManager::BindCommand(SDL_Scancode key, std::unique_ptr<Command> command)
    {
        m_pImpl->m_KeyBindings[key] = std::move(command);
    }

    void InputManager::BindCommand(ControllerButton button, std::unique_ptr<Command> command)
    {
        m_pImpl->m_ButtonBindings[static_cast<unsigned short>(button)] = std::move(command);
    }
